...
function getPathSums(root: Node) {
  ...
  while (true) {
    ...
    if (!top.node) {
      ...
      stack.unshift(new Frame(top.node.left));
    } else if (top.count === 1) {
      stack.unshift(new Frame(top.node.right));
    } else if (top.count === 2) {
      let popped = stack.shift();
      if (!popped.sums.length) popped.sums.push(popped.node.val);
      if (!stack.length) break;
      top = stack[0];
      for (let sum of popped.sums) top.sums.push(sum + top.node.val);
      top.count++;
    }
  }
  return primer.sums;
